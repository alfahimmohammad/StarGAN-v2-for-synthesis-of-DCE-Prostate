...
#             print(f'x_src: {x_src.shape}, y_src: {y_src.shape}, x_ref: {x_ref.shape}, y_ref: {y_ref.shape}')
...
class SliceDataProstate(Dataset):
    """
    A PyTorch Dataset that provides access to MR image slices.
    """

    def __init__(self, root, source = ['DCE_01', 'DCE_02'], target = ['DCE_02', 'DCE_03'], mode='train', train_test_split = 0.8):
            ...
            for j in range(i,len(target)):
                for fname in imgs:
                    with h5py.File(fname,'r') as hf:
                        ...
                        self.examples += [(fname, slice, source[i], target[j]) for slice in range(num_slices)]
    ...
    def __getitem__(self, i):
        ...
        fname, slice, src_domain, trg_domain = self.examples[i]
        with h5py.File(fname, 'r') as data:    
            t2 = data['T2'][:,:,:,slice].astype(np.float32)
            adc = data['ADC'][:,:,:,slice].astype(np.float32)
            pd = data['PD'][:,:,:,slice].astype(np.float32)
            dce_01 = data['DCE_01'][0,:,:,slice].astype(np.float32)
            dce_02 = data['DCE_02'][0,:,:,slice].astype(np.float32)
            dce_03 = data['DCE_03'][0,:,:,slice].astype(np.float32)
#             print(f'dce_01: {dce_01.shape}')
            
            dce_01 = cv2.resize(dce_01,(240,240),cv2.INTER_CUBIC)
            dce_02 = cv2.resize(dce_02,(240,240),cv2.INTER_CUBIC)
            dce_03 = cv2.resize(dce_03,(240,240),cv2.INTER_CUBIC)
            
#             print(f'dce_01: {dce_01.shape}')
            
        if src_domain=='DCE_01':
            c_src = torch.tensor([0])
            src_img = dce_01
        elif src_domain=='DCE_02':
            c_src = torch.tensor([1])
            src_img = dce_02
        elif src_domain=='DCE_03':
            c_src = torch.tensor([2])
            src_img = dce_03
        
        if trg_domain=='DCE_01':
            c_trg = torch.tensor([0])
            trg_img = dce_01
        elif trg_domain=='DCE_02':
            c_trg = torch.tensor([1])
            trg_img = dce_02
        elif trg_domain=='DCE_03':
            c_trg = torch.tensor([2])
            trg_img = dce_03
        
        if self.mode == 'train':
            return src_img, c_src, trg_img, c_trg
        else:
            return src_img, c_src, trg_img, c_trg, src_domain, trg_domain
